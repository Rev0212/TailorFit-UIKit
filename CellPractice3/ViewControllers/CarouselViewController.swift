...
class CarouselViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private func setupCollectionViews() {
            let photoLayout = CarouselFlowLayout()
            photoCollectionView = UICollectionView(frame: .zero, collectionViewLayout: photoLayout)
            photoCollectionView.register(PhotoCollectionViewCell1.self, forCellWithReuseIdentifier: "PhotoCell")
            photoCollectionView.dataSource = self
            photoCollectionView.delegate = self
            photoCollectionView.backgroundColor = .clear
            photoCollectionView.translatesAutoresizingMaskIntoConstraints = false

            let apparelLayout = CarouselFlowLayout()
            apparelCollectionView = UICollectionView(frame: .zero, collectionViewLayout: apparelLayout)
            apparelCollectionView.register(ApparelCollectionViewCell1.self, forCellWithReuseIdentifier: "ApparelCell")
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            apparelCollectionView.heightAnchor.constraint(equalToConstant: 200),
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return section == 0 ? photoImages.count + 1 : apparelImages.count + 1
    }
